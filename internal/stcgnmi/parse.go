...
func (p *rowParser) parseRow(r stcweb.StatRow) (map[string]*uint64, map[string]*float32, error) {
	name := r[p.nameKey]
	if name == "" {
...
func parsePortStats(t stcweb.StatTable) ([]*portRow, error) {
	const (
		nameKey      = "Port Name"
...
func parsePortCPUStats(t stcweb.StatTable) ([]*portCPURow, error) {
	const (
...
func parseFlowStats(t stcweb.StatTable) ([]*flowRow, error) {
	return parseFlowStatsHelp(t, "")
}

func parseFlowStatsHelp(t stcweb.StatTable, overrideNameKey string) ([]*flowRow, error) {
	const (
...
func parseEgressStats(t stcweb.StatTable) ([]*egressRow, error) {
